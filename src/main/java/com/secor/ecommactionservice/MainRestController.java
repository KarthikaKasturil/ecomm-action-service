    ...
    public ResponseEntity<?> createOrder(@RequestBody Order order) {
        ...
        if (product == null) {
            ...
            if (inventoryItem == null) {
                ...
                Payment savedPayment = actionService.savePayment(payment);
                LOG.info("Payment created: {}", savedPayment);
                return ResponseEntity.ok("Order initiated successfully with order Id: " + savedOrder.getOrderId() + " and created payment with paymentId: " + savedPayment.getPaymentId());
            }
        }
    }
        ...
        if (payment != null) {
            if("PENDING".equalsIgnoreCase(payment.getStatus())){
                ...
                if ("COMPLETED".equalsIgnoreCase(updatedPayment.getStatus())) {
                    ...
                    InventoryItem inventoryItem = actionService.checkInventory(orderDetails.getProductId());
                    ...
                    if (inventoryItem != null && inventoryItem.getQuantity() >= orderDetails.getQuantity()) {
                        ...
                        ResponseEntity<?> result = actionService.updateCustomerOrder(orderInfo);
                        LOG.info("Order creation: " + result.toString());
               ...
               return ResponseEntity.ok("Order is already processed with paymentId: "+ payment.getPaymentId());
            }
        }
